...
const removeContentType = (headers: HeadersInit): HeadersInit => {
  if (typeof headers === "object" && !Array.isArray(headers) && !(headers instanceof Headers)) {
    const { "Content-Type": _, ...cleanHeaders } = headers as Record<string, string>;
    return cleanHeaders;
  }
  return headers;
};
...
export const prepareRequest = (
  ...
  if (isFormData) {
    ...
    const builtHeaders = buildHeaders(baseHeaders, init?.headers, token, isPublic);
  ...
  const headers = buildHeaders(requestConfig.headers, init?.headers, token, isPublic);
...
export const handleErrorResponse = async <T>(
  ...
  return new ApiResponse(false, response.status, new ApiError([fallbackMessage]), responseMetadata);
};
